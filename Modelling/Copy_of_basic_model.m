...
dor = input('Deploy or Retract? ', 's');
dor = lower(dor);

while dor ~= "deploy" && dor ~= "retract"
...
if dor == "deploy"
    ...
    for a0 = 1:length(G_r)
        ...
        for a1 = 1:length(c)
            ...
            motor = @(t,z) ((-(k*z(2)*G_r(a0))+T_s)*G_r(a0))/(m*radius(t,z)^2);

            damping = @(t,z) -c(a1)*z(2)/(m*radius(t,z)^2);

            gravity = @(t,z) -g*cos(z(1)-pi/2)/radius(t,z); % beyond 90 degrees, gravity = accelerating torque: is this included already?
...
if dor == "retract"
    ...
    for a0 = 1:length(G_r)
        ...
        for a1 = 1:length(c)
            ...
            gravity = @(t,z) -g*cos(z(1)-pi/2)/radius(t,z);
